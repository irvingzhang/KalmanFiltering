...
这是应用程序的主要头文件。它包括其他项目特定的头文件(包括 Resource.h)，并声明 CNonlinear_FilteringApp 应用程序类。

Nonlinear_Filtering.cpp
这是包含应用程序类 CNonlinear_FilteringApp 的主要应用程序源文件。

Nonlinear_Filtering.rc
这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源位于 2052 中。

res\Nonlinear_Filtering.ico
这是用作应用程序图标的图标文件。此图标包括在主要资源文件 Nonlinear_Filtering.rc 中。
...
Nonlinear_FilteringDoc.h，Nonlinear_FilteringDoc.cpp - 文档
这些文件包含 CNonlinear_FilteringDoc 类。编辑这些文件可以添加特殊文档数据并可实现文件保存和加载(通过 CNonlinear_FilteringDoc::Serialize)。

Nonlinear_FilteringView.h，Nonlinear_FilteringView.cpp - 文档的视图
这些文件包含 CNonlinear_FilteringView 类。
CNonlinear_FilteringView 对象用于查看 CNonlinear_FilteringDoc 对象。
...
StdAfx.h，StdAfx.cpp
这些文件用于生成名为 Nonlinear_Filtering.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
